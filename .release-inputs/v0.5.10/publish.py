...
for item in meta['parts']:
    chunk=(base/item['file']).read_bytes();assert hashlib.sha256(chunk).hexdigest()==item['sha256'];chunks.append(chunk)
asset.write_bytes(b''.join(chunks))
...
try:
    ...
    if matches:
        ...
        release=api(f'repos/{repo}/releases',dict(tag_name=meta['tag'],target_commitish=os.environ['GITHUB_SHA'],name='EATWF Launcher 0.5.10',body=(base/'RELEASE-NOTES.md').read_text(),draft=True,prerelease=True))
